    ...
    switch (action.type) {
        ...
        case SET_FIRST_PAGE:
            if (store.currentPage === 1)
                return ({ ...store, isFirstPage: true })
            else
                return ({ ...store, isFirstPage: false })

        case SET_LAST_PAGE:
            if (store.currentPage >= Math.ceil((action.payload + 1) / 5))
                return ({ ...store, isLastPage: true })
            else
                return ({ ...store, isLastPage: false })
...
export const setLastPageAction = (payload) => ({ type: SET_LAST_PAGE, payload })